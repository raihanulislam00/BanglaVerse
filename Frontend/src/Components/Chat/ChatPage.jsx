...
export default function ChatPage() {
  ...
  const downloadChat = () => {
    const chatContent = messages
      .map(msg => `${msg.role === 'user' ? 'User' : 'BanglaVerse AI'}: ${msg.content}`)
      .join('\n\n');
    const blob = new Blob([chatContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'banglaverse-chat.txt';
          ...
          <div className="mx-auto max-w-4xl space-y-6">
            ...
            {messages.length === 0 && !currentResponse && (
              <div className="text-center py-16">
                ...
                <h1 className={`text-4xl font-bold bg-gradient-to-r from-red-primary via-accent-gold to-green-primary bg-clip-text text-transparent font-exo ${animationsEnabled ? 'animate-gradient-x' : ''} mb-2`}>
                  BanglaVerse AI
                </h1>
                <h2 className={`text-2xl font-bold ${darkMode ? 'text-gray-300' : 'text-gray-700'} bangla-text mb-4`}>
                  বাংলাভার্স এআই
                </h2>
                <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-600'} font-poppins max-w-2xl mx-auto leading-relaxed bangla-text mb-4`}>
                  আপনার সম্পূর্ণ বাংলা ভাষার সহযোগী। প্রশ্ন করুন, কথোপকথন করুন, এবং বাংলা ভাষার মহাবিশ্ব আবিষ্কার করুন।
                </p>
                ...
                <div className="flex items-center justify-center gap-4 mb-8">
                  ...
                  <div className={`flex items-center gap-2 px-4 py-2 ${darkMode ? 'bg-yellow-900/50' : 'bg-accent-gold/20'} rounded-xl`}>
                    ...
                    <span className={`text-sm font-bold ${darkMode ? 'text-yellow-400' : 'text-accent-gold'}`}>BanglaVerse AI</span>
                  </div>
                </div>
